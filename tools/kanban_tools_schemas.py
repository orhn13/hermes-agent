...
KANBAN_REAP_SCHEMA = _schema(
    ...
    (
        ...
        "'done', already terminal), OR be a 'ready' card whose assignee is "
        "not an installed profile — those can never dispatch, so 'archive' "
        "is safe on them too (only 'archive', not 'unblock': there is no "
        "'ready' state to unblock back into). Only 'archive' or 'unblock' "
        "are allowed (never 'complete', never a body/result rewrite — it "
        "cannot forge a success signal onto someone else's card), a reason "
        "is mandatory and is recorded on the target's own event log for "
        "audit, and the 'board' parameter is IGNORED — reap always targets "
        "whichever board your own task lives on, never an explicit override, "
        "so it can't reach across boards or tenants. Archiving is TERMINAL, "
        "same as everywhere else in Hermes (there is no unarchive) — "
        "'unblock' only works on a card that is still 'blocked'/'scheduled', "
        "never on one you already archived. If you are unsure whether a "
        "card is truly stale, use 'unblock' first (fully reversible: puts "
        "it back in the normal ready/todo flow) rather than reaching for "
        "'archive'."
    ),
    {
        "task_id": _prop("string", "The blocked/scheduled task id to retire or revive, "
                          "or a 'ready' task whose assignee cannot dispatch (archive only). "
